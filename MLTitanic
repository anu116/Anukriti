...
sbn.barplot(x="Sex", y="Survived", data=train) #all the elements are compared with the utpit
plt.show()
...
print("Percentage of Pclass = 1 who survived:", train["Survived"][train["Pclass"] == 1].value_counts())
...
train["Age"] = train["Age"].fillna(-0.5)#places where null value are available. Fillna finds those places
                                        #
...
train["CabinBool"] = (train["Cabin"].notnull().astype('int'))#boolean gets converted to int.
...
for dataset in combine:
    ...
    #expand=false terminates search once the data is found
    #+=one or more
    #*=0 or more
...
for dataset in combine:
    dataset['Title'] = dataset['Title'].map(title_mapping)#data can be reversed using map
...
for x in range(len(test["Fare"])):
    if pd.isnull(test["Fare"][x]):
        ...
        test["Fare"][x] = round(train[ train["Pclass"] == pclass ]["Fare"].mean(), 2)#rounded off to 2 places
...
output.to_csv('submission.csv', index=False)#no row heading when index=false
